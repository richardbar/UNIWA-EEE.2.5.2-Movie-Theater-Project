...
public sealed class MovieService : IMovieService
{
    ...
    public async Task<bool> CreateAsync(Domain.Entities.Movie movie)
    {
    ...
    public async Task<Domain.Entities.Movie?> GetAsync(Guid id)
    {
        var movieDto = await _movieRepository.GetAsync(id);
        return movieDto?.ToMovie();
    }

    public async Task<IEnumerable<Domain.Entities.Movie>> GetAllAsync()
    {
        var movieDtos = await _movieRepository.GetAllAsync();
        return movieDtos.Select(movieDto => movieDto.ToMovie());
    }

    public async Task<bool> UpdateAsync(Domain.Entities.Movie movie)
    {
        var movieDto = movie.ToMovieDto();
