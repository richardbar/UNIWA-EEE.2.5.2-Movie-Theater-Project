...
public class MovieViewingService : IMovieViewingService
{
    ...
    public async Task<bool> CreateAsync(Domain.Entities.MovieViewing movieViewing)
    {
    ...
    public async Task<Domain.Entities.MovieViewing?> GetAsync(Guid id)
    {
        var movieVewingrDto = await _movieViewingRepository.GetAsync(id);
        return movieVewingrDto?.ToMovieViewing();
    }

    public async Task<IEnumerable<Domain.Entities.MovieViewing>> GetAllAsync()
    {
